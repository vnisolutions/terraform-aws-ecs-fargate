...
resource "aws_appautoscaling_policy" "memory_policy" {
  name               = "${var.env}-${var.project_name}-${var.service_name}-memory-policy"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.aws_appautoscaling_target.resource_id
  scalable_dimension = aws_appautoscaling_target.aws_appautoscaling_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.aws_appautoscaling_target.service_namespace
...
resource "aws_appautoscaling_policy" "cpu_policy" {
  name               = "${var.env}-${var.project_name}-${var.service_name}-cpu-policy"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.aws_appautoscaling_target.resource_id
  scalable_dimension = aws_appautoscaling_target.aws_appautoscaling_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.aws_appautoscaling_target.service_namespace
...
resource "aws_appautoscaling_policy" "memory_policy_custom" {
  name               = "${var.env}-${var.project_name}-${var.service_name}-memory-policy-custom"
...
resource "aws_appautoscaling_policy" "cpu_policy_custom" {
  name               = "${var.env}-${var.project_name}-${var.service_name}-cpu-policy-custom"
  policy_type        = "StepScaling"
  resource_id        = aws_appautoscaling_target.aws_appautoscaling_target.resource_id
  scalable_dimension = aws_appautoscaling_target.aws_appautoscaling_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.aws_appautoscaling_target.service_namespace
...
resource "aws_cloudwatch_metric_alarm" "cpu_usage_high" {
  alarm_name          = "${var.env}-${var.project_name}-${var.service_name}-cpu-usage-high"
...
resource "aws_cloudwatch_metric_alarm" "memory_usage_high" {
  alarm_name          = "${var.env}-${var.project_name}-${var.service_name}-memory-usage-high"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
